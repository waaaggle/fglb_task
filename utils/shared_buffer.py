...
from fglb_task.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class SharedReplayBuffer(object):
    ...
    """
    # episode_length：一条轨迹的步数
    # n_rollout_threads：环境进程数（并行采样）
    # num_agents：环境中智能体数
    # step：当前写入的步数索引
    # available_actions：可用动作mask（针对不同行为空间）
    #
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
        ...
        :param rnn_states_actor: (np.ndarray) RNN states for actor fglb_dut.
        :param rnn_states_critic: (np.ndarray) RNN states for critic fglb_dut.
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        #self.obs.shape的形状为(T+1, N, M, obs_dim)
        #self.obs[self.step] = share_obs.copy()写法实际上是加入一个维度为(N, M, obs_dim)的样本，obs第一维代表step数量
    ...
    #模型更新后，将最后一个时间步的数据复制到缓冲区的第一个时间步，以确保连续性
    ...
    #为 Hanabi 设计的更新后处理，仅复制部分数据（如 RNN 状态和掩码）
    ...
    #计算回报（returns）和优势估计（advantages），支持 GAE（广义优势估计）和 PopArt/ValueNorm 归一化。
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        """
        if self._use_proper_time_limits:     #是否考虑因时间限制而终止
            if self._use_gae:  #使用GAE
                self.value_preds[-1] = next_value
                gae = 0
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:  #使用 PopArt 或 ValueNorm，对值函数进行去归一化处理。
                        # step + 1
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        ...
                        #不使用 GAE，直接计算折扣回报：returns[t] = rewards[t] + gamma * returns[t+1] * masks[t+1]。
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + self.rewards[
                            step]) * self.bad_masks[step + 1] \
    ...
    #专为 Transformer 模型设计，考虑了智能体维度的打乱（通过 _shuffle_agent_grid）。
    ...
    #用于分块的 RNN 训练，将时间序列分割为长度为 data_chunk_length 的片段
