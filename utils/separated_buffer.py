...
#将张量 x 的前两维（时间步 T 和并行环境数 N）展平为 T * N，保持其他维度不变。
def _flatten(T, N, x):
    return x.reshape(T * N, *x.shape[2:])

#将张量 x 的维度从 (T, N, ...) 转换为 (N, T, ...) 并展平为 (N*T, ...)，适用于 RNN 或其他需要时间序列的场景。
def _cast(x):
...
class SeparatedReplayBuffer(object):
    ...
    #在模型更新后，将最后一个时间步的数据复制到缓冲区的第一个时间步，以确保连续性
    ...
    #计算回报（returns）和优势估计（advantages），支持 GAE 和 PopArt/ValueNorm 归一化。
    ...
    #分块的 RNN 训练，将时间序列分割为长度为 data_chunk_length 的片段。
